...
**本番URL**: https://hatofes-ecc25.web.app
**カスタムドメイン**: app.hatofes.com (CloudFlare管理)
**GitHub**: https://github.com/ehari-nishiki/hatofes-app-2026

---

## 📌 重要な最新アップデート (2026-02-27)

### 🎨 UI/UX大幅改善 ✅

1. **SVGアイコン完全統一**
   - テトリス、ガチャ、ラジオ、Q&Aなど全てのページで絵文字→SVGアイコンに置き換え
   - グラデーション対応アイコンコンポーネント作成
   - `src/components/ui/Icon.tsx`: 統一されたアイコンシステム
   - `src/icon/`: オリジナルSVGファイル保存

2. **三役Q&A機能拡張** ✅
   - **複数回答対応**: 三役それぞれが回答を追加可能
   - **回答編集機能**: 既存の回答を編集できる
   - **回答削除機能**: 不要な回答を削除可能
   - 既存の単一回答との後方互換性維持

3. **ホーム画面UI改善** ✅
   - "Show detail →" を矢印アイコンに変更（ポイント・レベル）
   - ガチャ・テトリスバナーに色付き背景追加（ラジオ・Q&Aと統一）
   - ViewMoreButtonの白い線削除（よりクリーンなデザイン）
...
5. **デザインコンポーネント整備** ✅
   - `src/components/ui/Button.tsx`: 統一されたボタンシステム
     - Button, LinkButton, ViewMoreButton
     - PointBadge, CountBadge
   - 全体的なデザイン一貫性向上

---

## 📌 以前のアップデート (2026-02-06 ~ 2026-02-07)
...
- [x] ステップ式新規登録
- [x] ホーム画面（レベル・ポイント・通知・タスク・ミッション・SVGアイコン統一）✅
...
- [x] ガチャ（SF演出・1連/10連・紙吹雪・SVGアイコン）✅
- [x] テトリス（7-bag・ホールド・逆回転・ランキング・SVGアイコン）✅
- [x] 三役Q&A（質問投稿・いいね・複数回答表示）✅
- [x] プロフィール・ログアウト（アイコン化）・ユーザーネーム変更
...
### 三役機能
- [x] Q&A回答（複数回答対応・編集・削除・ピン留め）✅

---

## 🚧 ストレージ・メディア対応の課題

### Firebase Storageの問題点

Firebase Storageは**画像・動画のホスティングに不向き**:
- **無料枠**: 5GB/月、転送量1GB/日
- **コスト**: 1500人規模で画像アップロードすると即超過
- **想定問題**: ユーザーアイコン変更、画像投稿、ショート動画等
...
❌ デメリット:
  - Cloudflareアカウント・設定が必要
  - Cloud Function経由でアップロードURL生成が必要
```
...
**実装例**: 鳩ラジ、ショート動画をYouTube限定公開で配信

#### 3. **外部画像ホスティング**
```
サービス例: Imgur, imgBB, ImageKit
...
| 機能 | ストレージ必要性 | 推奨方法 |
|------|----------------|----------|
| ユーザーアイコン変更 | 必須 (256x256, 500KB制限) | Cloudflare R2 |
| 画像投稿システム | 必須 (枚数制限推奨) | Cloudflare R2 |
| ショート動画 | 任意 | YouTube埋め込み or 諦める |

### ⚠️ 実装時の注意事項

1. **Cloudflare R2を使う場合**:
   - `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` を環境変数に設定
   - Cloud Functionでプリサインドアップロード URL生成
   - クライアントから直接R2にアップロード（Firebase経由せず）
   - アップロード完了後、URLをFirestoreに保存

2. **画像サイズ制限**:
   - ユーザーアイコン: 256x256px、500KB以下
   - 投稿画像: 1920x1080px、2MB以下
   - クライアント側でリサイズ（`canvas` API使用）
...
**現時点では実装を保留し、必要性を再検討することを推奨**:
- 文化祭期間は2ヶ月と短期
- ユーザーアイコンはランダム生成で十分
- 画像投稿は管理者のみで対応可能

**どうしても必要な場合**: Cloudflare R2の設定から開始
...
| 優先度 | 機能 | 備考 |
|--------|------|------|
| 高 | カウントダウンタイマー | 文化祭当日までのカウントダウン |
| 高 | ブース一覧・マップ | 当日用機能 |
| 高 | イベントスケジュール | 当日用機能 |
| 中 | 鳩ラジ (校内ラジオ) | YouTube埋め込み推奨 |
| 中 | アンケート拡張 | 画像選択、スライダー、条件分岐 |
| 中 | プッシュ通知 (FCM) | |
| 低 | ユーザーアイコン変更 | Cloudflare R2必須 |
| 低 | 画像投稿システム | Cloudflare R2必須 |
| 低 | ショート動画 | YouTube埋め込み推奨 |
| 低 | QRコードスタンプラリー | ブース訪問でスタンプ |
| 低 | authErrorsログ削除ジョブ | 30日以上のログを自動削除 |
...
| UIコンポーネント | `src/components/ui/Icon.tsx` (SVGアイコン統一) 🆕 |
| UIコンポーネント | `src/components/ui/Button.tsx` (ボタン統一) 🆕 |
| アイコンファイル | `src/icon/*.svg` (オリジナルSVG) 🆕 |
| ヘッダー | `src/components/layout/AppHeader.tsx` (ログアウトアイコン) |
| 三役Q&A | `src/pages/executive/ExecutiveDashboard.tsx` (複数回答対応) |
| セキュリティルール | `firestore.rules` |
| Cloud Functions | `functions/src/index.ts` (21関数) |
| 開発ガイド | `CLAUDE.md` |
| 引き継ぎ資料 | `HANDOVER.md` (本ドキュメント) |
...
### 2026-02-27
- SVGアイコン完全統一（テトリス、ガチャ、ラジオ、Q&A）
- Icon.tsx / Button.tsx コンポーネント作成
- 三役Q&A複数回答対応 + 編集・削除機能
- ホーム画面UI改善（矢印アイコン、色付き背景、白線削除）
- ログアウトボタンをアイコン化
- ストレージ・メディア対応の課題整理
